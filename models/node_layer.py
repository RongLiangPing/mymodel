...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, concat=True):
        ...
        self.a = nn.Parameter(torch.empty(size=(2*out_features, 1)))    #a是shape为(2×out_features',1)的张量，为注意力参数
    ...
    def forward(self, h, adj):
        ...
        attention = torch.where(adj > 0, e, zero_vec)   #注意力参数矩阵，就是分子。np.where(condition, x, y),满足条件(condition)，输出x，不满足输出y. 
    ...
    def node_prepare_attentional_mechanism_input(self, Wh):
        ...
        # 下面创建了两个矩阵，它们的行中包含按不同顺序排列的嵌入。
        # (e stands for embedding)
        # 这些是第一个矩阵（Wh_repeated_in_chunks）的行。
        # e1, e1, ..., e1,            e2, e2, ..., e2,            ..., eN, eN, ..., eN
        # '-------------' -> N times  '-------------' -> N times       '-------------' -> N times
        # 
        # 这些是第二个矩阵（Wh_repeated_alternating）的行：
...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        edge_e = torch.exp(-self.leakyrelu(self.a.mm(edge_h).squeeze()))    #这里为什么加了-号，我不知道，反正原文没有。去掉符号最终准确率为0.839，不去掉就是0.84
